...
- **Parallel Processing**: Process multiple samples simultaneously with GNU Parallel
...
3. **GNU Parallel** - For parallel sample processing
4. **Perl** - For species matrix generation
5. **mail** (optional) - For email notifications
...
```bash
sudo apt-get update
sudo apt-get install seqtk parallel perl mailutils
```

**CentOS/RHEL:**
```bash
sudo yum install epel-release
sudo yum install seqtk parallel perl mailx
```
...
**What this does:**
1. Copies the archive to your home directory [check point 1 is to avoid error;Don't remove it]
2. Extracts SPINGO and database files
3. Verifies SPINGO installation
4. Builds the RDP reference database
...
**🔗 [https://github.com/GuyAllard/SPINGO](https://github.com/GuyAllard/SPINGO)**
...
**Parameters:**
- `<study_name>`: A name for your analysis (used in output filenames)
- `<threads>`: Number of CPU threads to use per sample
...
- Paired-end files must have `_1` and `_2` suffixes
...
| 2 | SCORE | Similarity score (k-mer match fraction) |
| 3 | L1 | Level 1 taxonomic assignment (e.g., clostridium group) |
| 4 | L1_BS | Bootstrap score for L1 |
| 5 | L2 | Level 2 assignment (genus) |
| 6 | L2_BS | Bootstrap score for L2 |
| 7 | L3 | Level 3 assignment (species) |
| 8 | L3_BS | Bootstrap score for L3 |

### Species Abundance Matrix

**File:** `species_matrix_<study_name>.txt` (single-end) or `abundance_table_<study_name>.txt` (paired-end)
...
### Adjusting Parallel Jobs

By default, scripts process samples in parallel:
- **Single-end**: 2 jobs simultaneously (line 88)
- **Paired-end**: 1 job at a time (line 87)

To modify, edit the `-j` parameter:
...
The scripts use these SPINGO parameters:
- `-w`: Write index file for faster subsequent runs
- `-d`: Reference database path
- `-p`: Number of threads per sample
- `-i`: Input file (streaming from stdin)
...
> Dinesh Palanimuthu. (2024). SPINGO Pipeline: Automated 16S Microbiome Analysis (Version 1.0.0) [Software]. Available from: https://github.com/YOUR_USERNAME/spingo-pipeline
...
- **SPINGO**: [https://github.com/GuyAllard/SPINGO](https://github.com/GuyAllard/SPINGO)
...
- **RDP Database**: [https://rdp.cme.msu.edu/](https://rdp.cme.msu.edu/)
...
- **SPINGO itself**: See the [SPINGO GitHub repository](https://github.com/GuyAllard/SPINGO)

## 🔄 Version History

- **v1.0** (2024): Initial release
  - Single-end and paired-end support
  - Parallel processing added but dormant
    [user himself can activate it by changing "-j 1" to "-j <number of parallel jobs>"]
...
graph TD
    ...
    InstallDeps --> Deps[Install: seqtk, GNU Parallel, perl, mail]
    Deps --> ConfigScripts[⚙️ Configure Pipeline Scripts]
    
    ConfigScripts --> UpdatePaths[Update SPINGO paths in scripts]
    UpdatePaths --> UpdateEmail[Update email address optional]
    UpdateEmail --> DataType{Data Type?}
    ...
    Output --> MatrixFile[Matrix: abundance_table_STUDY.txt or species_matrix_STUDY.txt]
